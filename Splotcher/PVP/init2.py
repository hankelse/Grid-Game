## PVP where players can not move on the splotches of others. 
# If they hit the other player they win.
...
class Character:
    ...
    def get_moves(self, keys, opponent):
        self.x, self.y = round(self.x), round(self.y)
        is_new_move = False
        self.new_moves = []
        self.opponent_splotch_coords = []
        for splotch in opponent.splotches:
            self.opponent_splotch_coords.append([splotch.x, splotch.y])
        if self.x + 1 <= grid_size and right_key(keys, self.controls)== True:
            if  [self.x+1, self.y] not in self.opponent_splotch_coords or [self.x+1, self.y] == [opponent.x, opponent.y]:
        ...
        if self.y + 1 <= grid_size and down_key(keys, self.controls) == True :
            if [self.x, self.y+1] not in self.opponent_splotch_coords or [self.x, self.y+1] == [opponent.x, opponent.y]:
...
class Particle:
    def __init__ (self, x, y, x_direction, y_direction, speed, color, size):
        ...
        self.color, self.size = color, size*random.randint(1, 20)/10
    def move(self):
...
def destroy(winner, loser):
    ...
    particles = []
    for i in range(10): particles.append(Particle(loser.x, loser.y, winner.x_direction, winner.y_direction, particle_speed, loser.player_color, player_size/5))
...
player_move_duration = 5
particle_speed =  player_move_duration
